...
PrintWitchDamageAndRemove(witch)
{
	...
	if (GetTrieArray(witchTrie, witch_key, witch_dmg_array, MAXPLAYERS+2))
	{
		...
		{
			PrintToChatAll("\x01Witch had \x05%d\x01 health left", witch_remaining_health);
		}
		else
		{
			PrintToChatAll("\x01Witch");
		}
		for (new i = 0; i < array_size; i++)
		{
			...
			{
				PrintToChatAll("\x03%N - \x05%d\x01", current_client[CDE_client], current_client[CDE_damage]);
			}
			else
			{
				PrintToChatAll("\x03Unknown - \x05%d\x01", current_client[CDE_damage]);
			}
		}
		if (witch_dmg_array[0])
		{
			PrintToChatAll("\x03Infected - \x05%d\x01", witch_dmg_array[0]);
		}
	}
	SDKUnhook(witch, SDKHook_OnTakeDamagePost, OnTakeDamage_Post);
	RemoveFromTrie(witchTrie, witch_key);
