...
public ActivateVocalization(client)
{
	if(GetClientTeam(client) == TEAM_INFECTED)
	{
		...
		{
			VocalizerTimer[ZOMBIE_SMOKER] = CreateTimer(float(TimerValue), timer_PrepareHeardSmoker);
			bSmokerAlive = CountSpecialInfected(ZOMBIE_SMOKER);
		}

		if(infected == ZOMBIE_BOOMER && bBoomerAlive == 0)
		{
			VocalizerTimer[ZOMBIE_BOOMER] = CreateTimer(float(TimerValue), timer_PrepareHeardBoomer);
			bBoomerAlive = CountSpecialInfected(ZOMBIE_BOOMER);
		}
	}
}
...
public Action:timer_PrepareHeardSmoker(Handle:timer, any:client)
{
	...
	if(bSmokerAlive > 0)
		VocalizerTimer[ZOMBIE_SMOKER] = CreateTimer(20.0, timer_PrepareHeardSmoker);
}
...
public Action:timer_PrepareHeardBoomer(Handle:timer, any:client)
{
	...
	if(bBoomerAlive > 0)
		VocalizerTimer[ZOMBIE_BOOMER] = CreateTimer(20.0, timer_PrepareHeardBoomer);
}
...
stock CountSpecialInfected(type)
{
	...
	{
		if(IsClientInGame(i) && GetClientTeam(i) == TEAM_INFECTED && IsPlayerAlive(i) && GetEntProp(i, Prop_Send, "m_zombieClass") == type)
			int++;
	}
	return int;
}
